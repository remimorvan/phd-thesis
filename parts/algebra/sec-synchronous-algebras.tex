...
\begin{definition}
	...
	\AP$\intro*\bigdisunion \?X \defeq \bigcup_{\tau \in \types} X_\tau \times \{\tau\}$,
...
$\bigdisunion{\?A}$ by first merging units,
...
% 			$\LB$ and $\BL$ are
% 			$\ZnZ{p}$, $\ZnZ{q}$ and $\ZnZ{r}$ under addition, respectively.
% 		\item $(\?Z_{p,q,r})_{\LL\to\LB} = \ZnZ{p} \times \ZnZ{q}$ and \\
% 			$(\?Z_{p,q,r})_{\LL\to\BL} = \ZnZ{p} \times \ZnZ{r}$,
% 			with natural addition
% 		\item the units are the zeroes,
% 		\item $\dep$ identifies $\bar x \in \ZnZ{p}$ with
% 		$(\bar x, \bar 0) \in \ZnZ{p} \times \ZnZ{q}$
% 		and with $(\bar x, \bar 0) \in \ZnZ{p} \times \ZnZ{r}$,
% 		\item similarly, it identifies $\bar x \in \ZnZ{q}$ with
% 		$(\bar 0, \bar x) \in \ZnZ{p} \times \ZnZ{q}$
% 		and dually, it identifies  $\bar x \in \ZnZ{r}$ 
% 		with $(\bar 0, \bar x) \in \ZnZ{p} \times \ZnZ{r}$.
% 	\end{itemize}
...
\begin{example}[Group relations: {\Cref{ex:group-languages}, cont'd.}]
	...
	\begin{itemize}
		...
		\item the cyclic monoid $(\ZnZ{p},+)$ for "type" $\LB$,
		\item the cyclic monoid $(\ZnZ{q},+)$ for "type" $\BL$.
	\end{itemize}
	Moreover, the sets $Z_{\LL\to\LB}$ and $Z_{\LL\to\BL}$ are defined
	as $\ZnZ{p}$ and $\ZnZ{q}$, respectively.
	The product is addition---we identify $\type{0}{\LL}$ with the zero of $\ZnZ{p}$ and
	of $\ZnZ{q}$. We denote by $\bar k$ the equivalence class of $k\in \Z$ in
	$\ZnZ{n}$ when $n$ is clear from context.
	...
	\begin{align*}
		...
		\hphantom{\text{ or}}\big\},
	\end{align*}
	where $I \subseteq \ZnZ{p}$ and $J \subseteq \ZnZ{q}$ are such that $\bar 0 \not\in I$
	...
	and $\bar 0 \not\in J$ are "group relations": letting $G$ be the group $\ZnZ{p} \times \ZnZ{q}$,
...
\[
	...
	\type{\pair{bb}{\pad\pad}}{\LB} \congr{\+R} \type{\varepsilon}{\BL}\text{ and }
