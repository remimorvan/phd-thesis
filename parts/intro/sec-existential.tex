...
fragment of SQL and $\?B$ encodes a relational database.
...
\begin{align*}
	...
	\exists \textsf{duration}.\, 
	\exists \textsf{director}.\,
	\exists \textsf{room\_id}.\, \\
	& \hphantom{\land~} \textsc{Movies}(\textsf{movie\_id}, \textsf{title}, \textsf{duration}, \textsf{director}) \\
	& \land
...
\end{marginfigure}
Yet another characterization of these queries is as "conjunctive queries",
...
\begin{known}
	...
	exactly correspond to the evaluation of "conjunctive queries". 
...
Assuming that the "query@@sem" is fixed,
...
of the circuit does not even depend on $\?A$.%
\footnote{There is roughly one layer simulating the
...
run over a 2.9 TB database, will likely execute more instructions
...
\begin{itemize}
	...
	\item studying the parameterized complexity of evaluating SQL queries, when parameterized by
...
\footnote{In short, "tree-width" is a graph parameter that measures how far a graph is from a tree.
...
In short, the field of parameterized complexity
...
Let us mention the parameterized classes "FPT" and "W[1]", which are roughly
the parameterized equivalent of "P" and "NP".
The problem of whether a "graph@@undir" contains a "$k$-clique",
when parameterized by $k$, is known to be "W[1]"-complete:
...
It follows that the "homomorphism problem", parameterized in the
size of the "source structure", is "W1"-hard. Unfortunately, assuming that "W1" $\neq$ "FPT",%
\footnote{This is the parameterized counterpart of "P" $\neq$ "NP".}
...
\begin{itemize}
	...
	\footnote{To quote K-Maro: ``Donne-moi ton cœur, baby, ton core, baby...''}
...
since it might lead to performance gains
for \emph{every} evaluation of the query: this is why studying the
complexity of the evaluation problem parameterized by the size of
...
a "query@CQ" of "tree-width" at most $k$ is "FPT".\footnote{In fact,
...
Grohe showed that a class of "conjunctive queries" has "FPT" evaluation when parameterized in the 
...
\end{marginfigure}
Overall, the previous result proves that the theory of \emph{conjunctive queries} is well-understood.
...
in $\?B$ is at most%
\footnote{Indeed, this follows from the definition of a "homomorphism", by using a trivial induction on the "distance@@struct".}
the distance from $a$ to $a'$ in $\?A$.
...
"st" $b'$ is a descendant, at \emph{any} depth, of $b$.
...
\textsf{?workLabel}, \textsf{?date} and \textsf{?authorLabel} such that:
\begin{itemize}
...
\clearpage
We formalize the fundamental features of SPARQL as "conjunctive regular path queries":
...
in contrast to "eg" "first-order logic"---their semantics is more complex:
...
that given a "conjunctive regular path query" and $k\in\N$, we can
...
a "conjunctive regular path query", which mostly depends on how many "atoms"/edges it has,
and not so much on how complex these languages are.}
The ideas behind the two algorithms are in fact surprisingly different:
\begin{itemize}
	...
	\item in the second case---when union is allowed---, we build a canonical
		...
		The difficulty there lies in proving the existence of "maximal under-approximations",
...
One reason we resolve to using a brute-force algorithm is that it is
...
for proofs---, and to prove complexity lower bounds for the decision problems.
However, this only provides a sufficient condition that is often not
necessary. In particular, it fails at providing a \emph{simple} algorithm to test minimality, justifying why we resolved to using brute-force algorithms.
...
It leads to the question of deciding when a "query@CRPQ" is actually equivalent
...
\begin{known}
	...
	if a "conjunctive regular path query" is equivalent to
...
by finite unions of "queries@CRPQ" of "tree-width" at most $k$ of a given
"conjunctive regular path query".

\begin{contribution}
	Given $k\in \Np$ and a "conjunctive regular path query",
...
much harder than in the case of minimizing
...
\sidenote[][-5em]{The main motivation behind this is that the evaluation of "queries@CRPQ"
...
to fix it%
\sidenote[][-2em]{See the notions of "contractions@@2w" and "contracted path-width".}
actually leads to a unified solution, that handles both
the case of "tree-width" $k$ (including $k=1$) and "path-width".%
\sidenote[][-1em]{The order of presentation of these results
...
used in practice are often much simpler than the ones required
to obtain the "ExpSpace"-hardness of the equivalence problem.
Fortunately, "conjunctive regular path queries" over "simple regular expressions",
...
is "FPT" when parameterized by the size of the "query@@sem".%
\footnote{This result was in fact already known---but proven differently, with
an incomparable complexity (better preprocessing but worse polynomial exponent)---by
...
\begin{openproblemintro}
	...
	when parameterized by the size of the "query@CRPQ".
