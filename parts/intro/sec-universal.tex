\section[Constraint Satisfaction Always Ends Up in Universal Problems]{Everyone Who Wants to Do Constraint Satisfaction Always Ends Up in Universal Problems}
...
infinite structures. This is motivated by two facts: not only do infinite structures
naturally arise from computational models---the runs of a machine
...
after hearing Gödel expose his famous "incompleteness theorem" in 1930:
any \emph{effective} (recursively enumerable)
consistent theory that is expressive enough to express arithmetic
is incomplete---"ie" it contains statements that are neither valid (true in all models),
nor unsatisfiable (false in all models).
...
Around the same time, Tarski formalized Euclidean geometry as a first-order theory, and proved
...
\begin{contribution}
	...
	for any "finite structure" $\?B$, the "homomorphism problem" with "target structure" $\?B$
...
we provide two alternative proofs: a logical one---which is short but quite abstract---and
a graph-theoretical one---the algorithm is much more concrete, but the proof of correctness is 
quite long. This last proof actually provides new hindsights on an existing algorithm from the literature, known as the "hyperedge consistency algorithm@@finite".
...
\begin{contribution}
	Any non-trivial---in the sense of ``"non-first-order definable"''---
	property on the "configuration graph"
...
in the "$\AUT$/$\REC$-separability problem", which takes as input two "automatic relations"---namely the binary relations over finite words described by "synchronous automata"---and asks if they can be
...
We prove this problem to be equivalent to the one taking an "automatic graph" and asking
whether it has a finite "regular colouring", which amounts to testing
...
actually yields, when translated back to the vocabulary of separability,
...
fed inputs that correspond to valid encodings. In other words,
