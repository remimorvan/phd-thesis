...
this thesis, and forms the basis for the two-part division of this thesis.
...
Note that in both \Cref{ex:auto-as-rel},
...
\begin{example}[SQL queries]
	...
	a "relational structure", sending
	$\textsf{title}$ to $x$ and $\textsf{time}$ to $y$
...
as a "homomorphism problem", we will see in fact that this encoding
is of a different nature than the encodings of \Cref{ex:auto-as-rel,ex:sql-as-hom}.
...
\begin{example}[Sudoku grids]
	...
	To deal with prefilled grids, we add nine unary "predicates" $P_k$ ($k \in \lBrack 1,9\rBrack$),
