...
\todo{def decision pb, signature fixed + "input structure" and "target structure"}.
...
\begin{table}
	...
	\begin{tabular}{cccc}
		\toprule 
		& data & query & homomorphism problem \\ \midrule 
		...
		Ex~\ref{ex:sudoku-as-hom} & grid & is solvable? & $\textsf{data} \homto^? \textsf{query}$ \\
...
\end{table}
All problems of the type above can actually be seen as model-checking problem:
part of the input represents some \emph{data} (or model) and part of it represents a
\emph{query} (or logical question/specification).
...
\begin{itemize}
	\item the \emph{query} is encoded in the "input structure",
	...
	\item in the other cases, the \emph{data} is encoded in the "input structure"
		and the \emph{query} is the "target structure": we denote
