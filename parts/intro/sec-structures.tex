...
Homomorphisms capture the idea of maps preserving relational information from a mathematical structure to another.
...
Then, a "homomorphism" from a graph to another is a
function $f$ between their vertices that preserves
...
\begin{marginfigure}[-12em]
	...
	\begin{tikzpicture}
		...
		\node[font=\tiny, right=2em of 3] {\textsf{duration}};
...
\begin{example}[Sudoku grids]
	...
	complete Sudoku grids that extend $G$, with $f(\tup{i,j})$ giving
...
the encodings of these problems into "homomorphism problems" can be of two types:
\begin{itemize}
...
data is usually much larger than the query:
