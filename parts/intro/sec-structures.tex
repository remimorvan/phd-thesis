...
\subsection{Existentialism is a Database Theory}
...
Note that in this case, and assuming that the "query@@sem" if fixed,
then the naive algorithm to solve the "homomorphism problem"---consisting namely enumerating
...
SQL is well-understood: it amounts to, given a "finite $\sigma$-structure" $\?A$,
...
\subsection{Everyone Who Wants to Do Logic Programming Always Ends in Universal Problems}
