...
\begin{example}
	...
	todo:model theory rk after model-checking
\end{example}
...
\begin{proof}[Proof sketch]
	...
	% \[
	% 	\univStructSynchronous{2} \models \phi^{\•A}
	% 	\quad\text{"iff"}\quad
	% 	\?A \models \phi.
	% \]
	...
	TODO:NOT CLEAR.
	% We reduce from "equivalence of star-free expressions@@problem", which is
	% "Tower"-complete by \cite[\S~3.1]{Schmitz2016ComplexityHierarchies}.
	% This problem takes two star-free expressions (over an alphabet $\Sigma$ with at least two 
	% letters), defined by the grammar
	% \[
	% 	E \defeq a \mid \varepsilon \mid \emptyset \mid E + E \mid EE \mid \neg E,
	% \]
	% and ask if they define the same "regular language".
	% Given an expression $E$, we build a "first-order formula" $\phi_E(x,y)$ over $\signatureSynchronous{\2}$
\end{proof}
