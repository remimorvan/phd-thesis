...
\begin{figure*}
	...
	\caption{
		...
		\todo{Diego:same parity -> same parity length}
		\todo{Diego:add equal length}
		\todo{Diego:Sync -> Aut}
	}
\end{figure*}
...
see \Cref{fig:multitape-automaton-suffix}.
...
see \Cref{fig:multitape-automaton-subword}. 
...
but the automaton of \Cref{fig:multitape-automaton-suffix} is not: its
...
$T_{\textrm{sync}} \defeq \{1,2\}^* \cdot (\{1\}^* + \{2\}^*)$: we start by moving both
heads synchronously, and then only of them---and we stick to the choice we made.
...
\end{figure}
\todo{Diego:  don't see the point of shortening the names as in FUN-AL, etc. You have plenty of space! (true)}
