...
\begin{figure*}
	...
	\caption{
		...
		\todo{UPDATE: SUFFIX RELATION IS Deterministic TWO-WAY AND RATIONAL.}
	}
\end{figure*}
...
Despite multiple reviews, the mistake was never spotted by anyone. \todo{ IS IT?}}
For instance, we will see in \todo{addref} see that "deterministic two-way multitape automata"
...
which has received lots of attention in \todo{addref}.
\decisionproblem{
...
\footnote{\todo{ Again, insert some set-theoretic bullshit.}}
...
\todo{ add examples lexicographic \& Gray.}
...
\begin{proposition}[{\cite[Theorem 1]{BarceloHongLeLinNiskanen2019MonadicDecomposability}, see also \cite[Corollary 2.9]{BergstrasserGanardiLinZetzsche2022RamseyQuantifiers}}]
	...
	of \todo{addref-deterministic-rational-relations}.
...
\todo{ convince myself that this is indeed true.}
...
\todo{this is similar to origin semantics}
...
\todo{\url{https://arxiv.org/pdf/1205.0178} says that the model is not closed under complementation…}
Of course, since "equivalence" is already undecidable on "rational relations", it is also
undecidable of this class.

It also makes sense to look at their determinic models: informally, in a given state, we
...
for more details.}
In fact, "deterministic two-way rational relations" (resp. "two-way rational relations")
exactly correspond to the closure of "deterministic (one-way) rational relations"
(resp. "(one-way) rational relations") under union and intersection,
see "eg" \cite[Theorem~4.1, p.~136]{Chan1980ReversalBoundedComputations}
and \cite[Theorem~4.2, p.~137]{Chan1980ReversalBoundedComputations}.
...
"Two-way rational relations" are not closed under complement,
see "eg" \cite[Theorem~4.6, p.~148]{Chan1980ReversalBoundedComputations}.
\footnote{In fact \cite[Theorem~4.6, p.~148]{Chan1980ReversalBoundedComputations}
proves that there is a relation $\+R$ that is a "rational relation"
but not "deterministic two-way rational@@rel". Its complement }
They are "rational relations" that are not "deterministic two-way rational@@rel"
\cite[Theorem~4.6, p.~148]{Chan1980ReversalBoundedComputations}.
...
\begin{figure}	
	...
	\begin{tikzpicture}
		...
		\node[above left=of rec] (aut)
			{\textsc{~\\"automatic@@rel"}\\
			\textsf{\footnotesize
			\textcolor{cGreen}{bool}\esp
			\textcolor{cGreen}{incl}\esp
			\textcolor{cGreen}{equiv}}};
		\node[above right=of rec] (raut)
			{\textsc{~\\"right-automatic@@rel"}\\
			\textsf{\footnotesize
			\textcolor{cGreen}{bool}\esp
			\textcolor{cGreen}{incl}\esp
			\textcolor{cGreen}{equiv}}};
		\node[above right=of aut] (drat)
			{\textsc{"deterministic\\rational@@rel"}\\
			\textsf{\footnotesize
			\textcolor{cRed}{$\neg$bool}\esp
			\textcolor{cRed}{$\neg$incl}\esp
			\textcolor{cGreen}{equiv}}};
		\node[above left=of drat] (rat)
			{\textsc{~\\"rational@@rel"}\\
			\textsf{\footnotesize
			\textcolor{cRed}{$\neg$bool}\esp
			\textcolor{cRed}{$\neg$incl}\esp
			\textcolor{cRed}{$\neg$equiv}}};
		\node[above right=of drat] (d2rat)
			{\textsc{"deterministic two-way rational@@rel"}\\
			\textsf{\footnotesize
			\textcolor{cGreen}{bool}\esp
			\textcolor{cRed}{$\neg$incl}\esp
			\textcolor{cRed}{$\neg$equiv}}};
		\node[above right=of rat] (2rat)
...
\AP""Non-deterministic one-way transducers"",
...
\begin{center}
	\begin{tabular}{rccc}
		...
		& $a\cdot b^n \cdot a$ & $\mapsto$ & $a \cdot b^n \cdot a$ \qquad``\qquad.
	\end{tabular}
\end{center}
...
For instance, the intersection of "functional relations" and "two-way transductions"
collapses to "regular functions" \cite[Theorem 22, p.~243]{EngelfrietHoogeboom2001transduction}.
\todo{look at Figure 9 of EH01}
...
The equivalence problem was shown to be decidable by Gurari \cite[Theorem~1]{Gurari1982Equivalence}
...
Gauwin's habilitation \cite{Gauwin2020Transductions}: in particular,
we refer the reader to the splendid \cite[Figure~2.1, p.~16]{Gauwin2020Transductions}%
...
\todo{ update figure}
\todo{ in ccl open question \cite[Theorem~V.4.2]{Sakarovitch2009Elements} for polyregular.}