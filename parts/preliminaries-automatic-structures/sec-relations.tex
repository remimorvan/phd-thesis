...
by \AP$\intro*\Id$, are "automatic@@rel", but not "recognizable@@rel".
As stated before, this class extends the class of "recognizable relations".
...
\end{openproblem}
Pablo Barceló introduced me to this problem in October 2022,
which has been open for many years.
It goes back at least to 2006, when Choffrut and Grigorieff
...
the "automaton@@desync" recognizing is that, when reading an $a$ on the first tape,
...
\begin{claim}
	...
	one word $w \in (\Sigma_1\convolAlpha\cdots\convolAlpha\Sigma_k)^*$ "st" $\projWord(w) = \tup{w_1,\dotsc,w_k}$ and there is a run of $\+A$, starting at $q$ and labelled by $w$.%
...
the concatenation $u^{|u|}$ of $u$ as many times as there are letter in the word.
