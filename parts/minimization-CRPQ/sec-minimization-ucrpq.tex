...
\end{proposition}
\begin{proof}
	The following example is inspired from \cite[Example 1.2]{FigueiraMorvan2025SemanticTreeWidthLMCS} ("aka"
	\Cref{ex:CRPQ-tw3-stw2}).
	...
	But $\anexpansion_\zeta$ cannot satisfy the atom $x \atom{ab(bb)^*} z$.
...
Given a graph class $\+C$ and a "UCRPQ" $\Gamma$, we define $\AppInf{\Gamma}{\+C}$ to be the (infinite) set of all $\CRPQ[\+C]$ queries which are "contractions@@1w" of "CQs" "contained" in $\Gamma$. More formally:\footnote{Note that the "contraction@@1w" of a "CQ" is a "CRPQ": this is why $\AppInf{\Gamma}{\+C}$ is an infinite union of "CRPQs" and not of "CQs".}
\AP\phantomintro{\AppInf}
\begin{align*}
	\reintro*\AppInf{\Gamma}{\+C} \defeq \{
		\alpha \in \CRPQ[\+C] & \mid \exists \xi \in \Exp(\Gamma), \exists \eta \in \CQ \text{ "st" }
		\xi \homto \eta \\
		& \qquad \text{ and } \alpha \text{ is a "contraction@@1w" of } \eta
	\}.
\end{align*}
...
\begin{proposition}
	...
	$\AppInf{\Gamma}{\+C}$ is a "maximal under-approximation" of $\Gamma$ by $\infUCRPQ[\+C]$ queries, in the sense that:
	\begin{enumerate}
...
\begin{proof}
	...
	$\eta \in \CQ$ "st" $\xi \homto \eta$ and $\alpha$ is a "contraction@@1w" of $\eta$,
...
\begin{lemma}
	...
	Further, the membership $\delta \in^? \Delta$ can be tested in "NP". 
...
\begin{proof}
	...
	\[\vec{\alpha} = \tup{\rho, \eta, \alpha, h, \orig, \cont}\]
	consisting of:
	\begin{itemize}
		\item queries $\rho \in \Refin(\Gamma)$, $\eta \in \CRPQ$ and
			$\alpha \in \CRPQ[\+C]$,
		\item a "homomorphism" $h\colon \rho \homto \eta$,
		\item witnesses that $\alpha$ is a "contraction@@1w" of $\eta$, in the form of
			a function $\orig$ from $\vars{\alpha}$ to $\vars{\eta}$,
	...
	Then we will be able to "contract@@1w" $x_i$ as well as the "internal variables" of the preimage, obtaining an "explicit approximation" which contains $\vec{\alpha_1}$.
	...
	We now show that we can bound the "refinement length" of an "explicit approximation",
	...
	"atoms". It follows then, by the pigeon-hole principle, that we can bound the
...
\AP Let ""UCRPQ(SRE)"" ("resp" ""CRPQ(SRE)"") be the set of all "UCRPQs" ("resp" "CRPQs") whose languages are expressed via "positive simple regular expressions", as defined in \Cref{sec:prelim-db-sre}. We show that if we restrict the regular expressions we obtain a much better complexity for the "minimization problem" for "UCRPQs".
Throughout this chapter, we will shorten ``positive simple regular expression''
to ``"simple regular expression@@positive"''.

\begin{theorem}\AP\label{thm:minimization-SRE}
	The "minimization problem", restricted to the class of "UCRPQs" over "simple regular expressions@@positive", is "PiP2"-complete.
\end{theorem}
