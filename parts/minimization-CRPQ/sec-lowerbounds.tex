...
\label{sec:minimization-lowerbounds}
...
We shall use the hard instances of \Cref{prop:variation-figueira}
to show that the "minimization problem" for "CRPQs" is hard.
\begin{theorem}
	\AP\label{thm:minimization-lowerbound}
	The "minimization problem" for "CRPQs" is "ExpSpace"-hard.
	Further, there is a fixed alphabet "st" the problem of, given a "Boolean CRPQ"
	on this alphabet with only four variables, whether it is "equivalent"
...
\begin{proof}[Proof details]
	We use the reduction described in the proof sketch.
...
Given a \AP""class of CRPQs"" $\intro*\classCRPQ$, the \AP""$\classCRPQ$-canonization problem""
...
\bigskip
We show that assuming we can solve the "$\classCRPQ$-canonization problem" ("resp" "$\classCRPQ$-strong canonization problem"), then the "CRPQ" (resp. "UCRPQ") "minimization problem"
...
\begin{proof}
	...
	We let $i_1 < \dotsc < i_k$ denote the indices $i$ "st" $x_i \in \gamma_1$, so
	...
	\begin{align*}
		...
		\atom{b_{i_2+1}} f(x_{i_{2}+1}) \atom{b_{i_2+2}}
		\cdots \atom{b_{i_k}} f(x_{i_k}) \\[1em]
		& \hphantom{%
			\underbrace{f(x_0) \atom{b_1} f(x_1) \atom{b_2} \cdots \atom{b_{i_1}}}_{
				\text{end of an "atom refinement" of $\alpha_0$}
			}
	...
	\begin{align*}
		...
		f(x_{i_2})^l 
		\atom{\triangleleft_{\alpha_{1}}} f(x_{i_2}) \atom{\triangleright_{\alpha_{1}}} f(x_{i_2})^r \\[1em]
		& \atom{b_{i_2+1}} f(x_{i_{2}+1}) \atom{b_{i_2+2}} \cdots
		\atom{b_{i_k}}
		f(x_{i_k})^l \atom{\triangleleft_{\alpha_{k-1}}} f(x_{i_k}) \atom{\triangleright_{\alpha_{k}}} f(x_{i_k})^r \\[1em]
		& \underbrace{\atom{b_{i_k+1}} f(x_{i_{k}+1}) \atom{b_{i_k+2}} \cdots \atom{b_n} f(x_n)}_{
			\text{beginning of an "atom refinement" of $\alpha_k$}
		}
	\end{align*}
	...
	\begin{align*}
		...
		\atom{b_{i_1+1}} x_{i_{1}+1} \atom{b_{i_1+2}} \cdots \\
		\cdots \atom{b_{i_2}}
		x_{i_2}^l \atom{\triangleleft_{\alpha_1}} x_{i_2} \atom{\triangleright_{\alpha_{2}}} x_{i_2}'
		\atom{b_{i_2+1}} x_{i_{2}+1} \atom{b_{i_2+2}} \cdots \\
		\cdots \atom{b_{i_k}}
...
\begin{proof}[Proof of \Cref{lem:canonization-SREs}]
	...
	\[
		...
		\land 
		\Bigl(
			\bigwedge_{x \in \gamma'_1} x \atom{\marking_x} x
		\Bigr),
	\]
	and we let $\alphabetmarking \defeq \{\marking_x \mid x \in \gamma'_1\}$.
	...
	$f$ must send $x \in \delta_1 \subseteq D_1$
	onto $x \in D'_1$. We then claim that $f$ is "strong onto".
...
\begin{corollary}
	...
	\begin{enumerate}
		...
		\itemAP\label{pspace:forest} "PSpace"-hard when restricted to "forest-shaped CRPQs",
		or even a single "atom",
