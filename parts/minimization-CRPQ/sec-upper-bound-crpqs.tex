...
The proof of \Cref{thm:2expspace-min-crpqs} is based on a key lemma stated below. Intuitively, this lemma tells us that if a "CRPQ" $\gamma$ is "equivalent" to another "CRPQ" $\alpha$, then $\gamma$ is also equivalent to a "CRPQ" $\beta$, where $\beta$ has the same “shape” than $\alpha$ but the sizes of the NFAs appearing in $\beta$ are bounded by the size of $\gamma$. In particular, if $\gamma$  is equivalent to a  "CRPQ" with at most $k$ atoms, then it is equivalent to a "CRPQ" with at most $k$ atoms and NFAs whose size is bounded by a function of $k$, and hence the search space in the minimization problem becomes finite. A careful analysis of this idea yields our "2ExpSpace" upper bound.
...
We start by giving a proof sketch of this lemma before giving the full
detailed proof.
...
\begin{proof}[Proof sketch]
	...
	\phantomintro{\typeMin}
	\begin{align*}
		\AP\reintro*\typeMin{\gamma}{u} \defeq \{
		([u_1]_{\weakequiv{\gamma}},\dots,[u_\ell]_{\weakequiv{\gamma}}) & \mid \text{$\ell\leq (\nbvar{\gamma} +1)$,}\\
		& \quad \text{ and $u_1,\dots,u_\ell\in \A^*$ "st" $u=u_1\cdots u_\ell$} \}.
	\end{align*}
