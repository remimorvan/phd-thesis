...
\begin{theorem}
	...
	on this alphabet with only five variables, whether it is "equivalent"
...
We say that a "class@@CRPQ" $\classCRPQ$ is \AP""closed under disjoint conjunction""
if $\gamma \in \classCRPQ_{\A}$ and $\delta \in \classCRPQ_{\B}$
imply $\gamma \disconj \delta \in \classCRPQ_{\A \cup \B}$.
Moreover, we say that the class is \AP""closed under variable marking""
...
\begin{proof}[Proof of \Cref{thm:reduction-containment-to-variable-minimization}]
	...
	\[
		\delta_1() \defeq \gamma_1 \land \bigwedge_{x \in \gamma_1} x \atom{\marking_x} x
	\]
	where $\marking_x$ is a fresh letter for each $x \in \gamma_1$.
	...
	We claim first that for each $x \in \zeta$ there is a
	...
	Since $D_1$ contains a $\marking_x$-self loop for each $x\in \gamma_1$,
	...
	for each $x\in \gamma_1$.

	Now observe that no variable of $\zeta$ can be labelled by two $\marking_x$-self-loops
	with $x\in \gamma_1$. Indeed, $\gamma_1$ is "non-degenerate@@CRPQ", and so $\delta_1$
	...
	$\zeta = \gamma_1$ and for each $x \in \gamma_1$,
	...
	the "homomorphism" $Z \homto D_1 \oplus G_2$ must map $x \in Z$  
	to $x \in D_1$---because of the $\marking_x$-self-loop.
	...
	since $G'_2$ does not use any letter of the form $\marking_x$, $x\in \gamma_1$,
