...
A \AP""one-way internal path""\footnote{This definition comes from \cite[\S 7]{FigueiraMorvan2025SemanticTreeWidthLMCS}, 
...
\begin{figure}
	...
	\subfloat[A "CRPQ" $\gamma$.]{%
		...
		\includegraphics[width=.45\linewidth]{fig/minimization-crpq/example-crpq.pdf}
	}
	\hfill%
	\subfloat[An "expansion" $\xi$ of $\gamma$, together with its "segments".]{%
		\AP\label{fig:segments-expansion}%	
		\includegraphics[width=.31\linewidth]{fig/minimization-crpq/segments-expansion.pdf}
	}
	\hfill%
	\subfloat[The "segment graph" of $\xi$.]{%
		\AP\label{fig:segment-graph-expansion}%
		\includegraphics[width=.2\linewidth]{fig/minimization-crpq/segment-graph-expansion.pdf}
	}
	\caption{%
