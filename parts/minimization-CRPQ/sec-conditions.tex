...
\AP
We say that a (U)CRPQ is \AP""minimal"" if it is not equivalent to any (U)CRPQ having strictly fewer atoms.
...
\footnote{The endpoint of a "cyclic segment" can either be "internal@@variable" or not,
see \Cref{fig:segments}.}
We say that a "segment" as in \eqref{eq:path-internal} is \AP""incident@@segment""
to a variable $y$ if $y = x_i$ for some $i$.

\begin{figure}
	\centering
	\subfloat[The "segments" of $\gamma$---labels are omitted. Each "segment" has a different colour. "Internal variables" are the smaller circles.]{%
...
One simple (and tractable) way to make a query smaller is to `contract'
any two consecutive atoms sharing an "internal variable", and replacing them with
a single atom having the concatenation of the languages: we call this
new query \AP""contraction@@1w"".
A "CRPQ" $\gamma$ is \AP""fully contracted"" if it cannot be "contracted@@1w".
In other words, $\gamma$ is "fully contracted" "iff" $\nbseg{\gamma} = \nbatoms{\gamma}$.
A "contraction@@1w" of a "UCRPQ" is a "contraction@@1w" of a "CRPQ" therein (obtaining the "UCRPQ" where one "CRPQ" $\gamma$ was replaced by a "contraction@@1w" of $\gamma$). A "UCRPQ" is then \reintro{fully contracted} if each "CRPQ" therein is "fully contracted".

\begin{fact}
	\AP\label{fact:produce-fully-contracted}
	"Contractions@@1w" preserve "semantical equivalence". Further, from a "UCRPQ" $\Gamma$ one can produce, in polynomial time, an "equivalent" one that is "fully contracted" with $\nbseg{\Gamma}$ "atoms".
\end{fact}

In particular, if a "UCRPQ" $\Gamma$ is "minimal" then $\Gamma$ is "fully contracted" and $\nbseg{\Gamma} = \nbatoms{\Gamma}$;%
\footnote{As for $\nbseg{\Gamma}$, we define $\nbatoms{\Gamma}$ to be the maximum
of the number of "atoms" of a "disjunct" of $\Gamma$.}
...
In order to prove this result, we first need to mention a variation of
\Cref{prop:containment-figueira}.

\begin{proposition}[{Variation of \Cref{prop:containment-figueira}}]
...
\begin{proof}[Proof of \Cref{prop:lowerbound-non-redundant}]
...
\begin{figure}
	...
	\caption{
		\AP\label{fig:ex-equiv-queries}"Equivalent" "CRPQs". The leftmost query is "fully contracted", "non-redundant" and not "minimal", since it is "equivalent" to the rightmost query.
	}
\end{figure}
\begin{proof}
...
it has a "hom-minimal"%
\footnote{Naturally, we say that an "expansion" is "hom-minimal" whenever
its "canonical database" is "hom-minimal".}
"expansion" $\anexpansion \in \Exp(\Gamma)$ "st" $\nbseg{\core{\anexpansion}} = \nbatoms{\Gamma}$. We will next show that this is a sufficient condition for minimality.

We denote by \AP$\intro*\underlying{\gamma}$ the ""underlying graph"" of a CRPQ $\gamma$,
namely the directed graph whose vertices are variables of $\gamma$,
and with an edge from $x$ to $y\in \gamma$ whenever there is some atom from $x$ to $y$ in $\gamma$.

Informally, the segment graph $\reintro*\seggraph(\gamma)$ of a "CRPQ" $\gamma$ is the directed multigraph obtained by first taking the "underlying graph", and then replacing "segments" of $\gamma$ with edges, as illustrated in \Cref{fig:segment-graph}. The main motivation behind the notion of "segments" is 
...
\begin{proof}
	...
	\begin{enumerate}
		\item contracting some edges---corresponding to "atom refinements"
			for $\varepsilon$,
				 \item potentially removing isolated vertices, and
	...
	all operations of type (1) and (2). By construction,
