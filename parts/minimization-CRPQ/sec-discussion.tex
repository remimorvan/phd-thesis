...
Another classical algorithm for query answering for "(U)CRPQs" consists in, given a "query@CRPQ" $\gamma$ and a "graph database" $G$, to first guess a variable assignment $f: \vars{\gamma} \to \vertex{G}$ and then check, for each "atom" $x \atom{L} y$ of $\gamma$, that there is a path in $G$ from $f(x)$ to $f(y)$ with label in $L$. 
