...
\begin{proof}
	...
	Now, when the right-hand side holds, given a "automatic structure" $\+A$,
...
\begin{proof}
	...
	Given a "automatic structure" $\+A$, we can then effectively test if $f$ is defined on $\domainPres{\+A}$, and if $f$ defines a "homomorphism" from $\?A$ to $\?B$---see
...
\subsection{Undecidability of \,$\HomRegAutDec{\?B}$}
\AP\label{sec:undecidability-homreg}
