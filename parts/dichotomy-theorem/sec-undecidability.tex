...
\begin{itemize}
	\item For $\HomFinDec{\?B}$, we reduce the complement of "Connectivity in synchronous graphs",
		providing a "coRE"-lowerbound.
	\item For $\HomRegFinDec{\?B}$, we reduce "Regular Unconnectivity in synchronous graphs",
...
\decisionproblem{""Connectivity in synchronous graphs""}{
...
\begin{property}
	...
	arity at least 2, "Connectivity in synchronous graphs" is "RE"-complete.
\end{property}
...
\begin{lemma}
	...
	from the complement of "Connectivity in synchronous graphs" to $\HomAutDec{\marked{\?B}}$.
\end{lemma}

\begin{proof}
	\marginnote{TODO:Addfigure}%
	Given an instance $\langle \•G, s, t \rangle$ of "Connectivity in synchronous graphs",
...
By \Cref{prop:undecidability-connectivity}, the complement of "Connectivity in synchronous graphs"
...
\decisionproblem{""Regular Unconnectivity in synchronous graphs""}{
...
\begin{lemma}
	...
	Assume that $\sigma$ contains at least one "predicate" of arity at least 2.
	If $\?B$ does not have "finite duality", then there is a TODO:complexity reduction 
	from "Regular Unconnectivity in synchronous graphs"
	to $\HomRegAutDec{\marked{\?B}}$.
\end{lemma}

\begin{proof}
	Given an instance $\langle \•G, s, t \rangle$ of "Regular Unconnectivity in synchronous graphs",
	we first define the $\sigma$-structure $\?A$ with "rational presentation" $\•A$
...
\begin{lemma}
	...
	"Regular Unconnectivity in synchronous graphs" is "RE"-hard.
\end{lemma}
...
\begin{proof}
	...
	% We will prove that:
	% \begin{enumerate}
	% 	\item there is a reduction from "Restricted Membership for Deterministic Reversible Turing Machines" to "Regular Unconnectivity in synchronous graphs", and 
	% 	\item the former problem is "RE"-hard.
	% \end{enumerate}

	% \proofcase{Reduction from "Restricted Membership for Deterministic Reversible Turing Machines" to "Regular Unconnectivity in synchronous graphs".}
	% We map an instance
