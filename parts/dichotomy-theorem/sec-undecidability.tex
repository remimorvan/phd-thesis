...
\begin{proof}
	...
	\proofcase{$\HomRegAutDec{\?B}$ is "RE".} This is an easy generalization of
...
\begin{proof}[Proof of \Cref{prop:dichotomy-Equal-implies-HomDec}.]
	If $\HomAutDec{\?B} = \HomRegAutDec{\?B}$, then by \Cref{prop:dichotomy-general-upper-bounds},
...
\decisionproblem{""Connectivity in rational graphs""}{
	A "rational presentation of a directed graph" $\•G$,
...
\begin{proof}
	...
	\[
		...
		\;\big\}.
	\]
	To construct an "rational presentation" for this structure, see \Cref{sec:construction-automatic-presentations}.
	\begin{claim}
...
By \Cref{prop:undecidability-connectivity}, the complement of "Connectivity in rational graphs"
is "coRE"-complete, and assuming that $\sigma$ contains at least one "predicate" of arity 2,
it reduces by \Cref{lem:reduction-hom} to any problem $\HomAutDec{\marked{\?B}}$ when $\?B$ has "finite duality". In turns, by \Cref{prop:idempotent-core-preserves-csp-complexity}, it reduces to
...
\begin{corollary}
	...
	$\HomFinDec{-}$, and prove in \cite[Theorem 3.2]{LaroseTesson2009UniversalAlgebraCSP}
	that there is a "first-order reduction" from "Connectivity in Finite Graphs" to
	$\HomFinDec{\marked{\?B}}$ assuming that $\?B$ does not have finite duality.
	...
	this shows that $\HomFinDec{\?B}$ is "L"-hard under "first-order reductions".}%
	%%%
...
\subsection{\AP\label{sec:undecidability-homreg}%
	Undecidability of \,$\HomRegAutDec{\?B}$}
...
\decisionproblem{""Regular Unconnectivity in Rational Graphs""}{
	An "rational presentation" $\•G$ of a "directed graph" $\?G$,
	and two elements $s,t \in \Sigma^*$.
}{
...
We will first reduce this problem to $\HomRegAutDec{\?B}$,
...
\begin{lemma}
	...
	to $\HomRegAutDec{\marked{\?B}}$.
\end{lemma}

\begin{proof}
	Given an instance $\langle \•G, s, t \rangle$ of "regular unconnectivity in rational graphs",
	we first define the $\sigma$-structure $\?A$ with "rational presentation" $\•A$
	...
	to an "rational presentation" of \(\ConstrUndecHom{(\?A\prodstruct \iterstruct{\?B}{2})}\),
...
\begin{proof}
	...
	% We will prove that:
	% \begin{enumerate}
	% 	\item there is a reduction from "Restricted Membership for Deterministic Reversible Turing Machines" to "Regular Unconnectivity in rational graphs", and 
	% 	\item the former problem is "RE"-hard.
	% \end{enumerate}

	% \proofcase{Reduction from "Restricted Membership for Deterministic Reversible Turing Machines" to "Regular Unconnectivity in rational graphs".}
	% We map an instance
	...
	% Indeed, let $L' \subseteq \Gamma^*$ be a "regular language" "st" $q_0\cdot u_\top \in L'$ $q_0\cdot u_\bot \not\in L'$, "st" $L'$ is a union of "connected components" of the "configuration graph" of $\+M$.\footnote{Here, $\Gamma$ denotes the alphabet used in the "rational presentation" of the "configuration graph" of $\+M'$. Namely, $\Gamma = \Sigma \dcup \statesTM{\+M} \dcup (\statesTM{\+M'} \smallsetminus \statesTM{\+M})$.} 
	% Then let \[L
...
\begin{proof}
	Recall that $\HomAutDec{\?B} = \HomAutDec{\core{\?B}}$, so we assume "wlog"
	that $\?B$ is a "core".
	...
	a reduction from "regular unconnectivity in rational graphs" to
	$\HomRegAutDec{\marked{\?B}}$, which in turns
	reduces to $\HomRegAutDec{\?B}$ by \Cref{prop:idempotent-core-preserves-csp-complexity}
	since $\?B$ was assumed to be a "core".
	Indeed, "first-order reductions" preserves "regularity@@hom", by 
	todo:addref-synchronous-iff-first-order.
\end{proof}


% \subsection{\AP\label{sec:undecidability-finite-colourability}Undecidability of Finite Regular Colourability}

% In this subsection, we capitalize on \Cref{sec:undecidability-hom,sec:undecidability-homreg} to 
...
% \begin{proof}[Proof of \Cref{thm:finite-regular-colourability-undecidable}.]
% 	Similarly to \Cref{coro:lowerbound-homreg},
...
% 	\Cref{prop:finite-colourability-of-connected-graphs}---, and
% 	\eqref{item:finite-regular-colourability-undecidable-homcolour} 
% 	follows from \Cref{prop:finite-regular-colourability-as-homreg-pb}.

% 	\proofcase{Reduction \eqref{item:finite-regular-colourability-undecidable-icore}
% 	from $\restrConnected{\HomRegAutDec{\marked{\omegaCliquePres}}}$
% 	to $\restrConnected{\HomRegAutDec{\omegaCliquePres}}$.}
% 	TODO.
	
% 	\proofcase{Reduction \eqref{item:finite-regular-colourability-undecidable-link}
% 	from the restriction of "regular unconnectivity in rational graphs"
% 	to "connected graphs" to 
% 	$\restrConnected{\HomRegAutDec{\marked{\omegaCliquePres}}}$.}
% 	TODO.
% \end{proof}