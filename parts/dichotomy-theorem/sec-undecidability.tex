...
Define the \AP""$n$-link"" $\intro*\link{n}$ be the "$\sigma$-structure"\sidenote{From \cite[\S~2]{LaroseLotenTardif2007CharacterisationFOCSP}.} 
...
\begin{lemma}
	...
	\sidenote{TODO: Does the main theorem hold for degenerate signatures?}
...
\begin{proof}
	...
	obtained by replacing any edge by a "$1$-link".
	\begin{claim}
		\AP\label{claim:reduction-hom-from-graph-to-link}
		By construction, $\•G(s)$ and $\•G(t)$ are "connected" "iff"
...
\begin{corollary}
	\!\footnote{This follows from \Cref{prop:marking-preserves-csp-complexity,prop:undecidability-connectivity,lem:reduction-hom}.}%
