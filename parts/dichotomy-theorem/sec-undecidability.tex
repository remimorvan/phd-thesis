...
\begin{proof}
	By assumption, there exist "first-order formulas"
	...
	Now, when the right-hand side holds, given an "automatic presentation" $\+A$,
	for each $b\in \?B$, we let $\tilde\phi_b(x)$ be the "first-order formula" over $\signatureSynchronous{\Sigma}$ obtained from $\phi_b(x)$
	by substituting $\+R(x_1,\dotsc,x_k)$ for the "first-order formula"
