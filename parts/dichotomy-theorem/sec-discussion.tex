...
Putting \Cref{prop:finite-regular-colourability-as-homreg-pb} and \Cref{prop:finite-colourability-of-connected-graphs} together, we obtain the following corollary.
\begin{corollary}
...
"regular unconnectivity in automatic graphs" to
\[\HomRegAut{\marked{\omegaCliquePres}}\]
heavily uses the fact that the "source structure" can be "unconnected@@struct".
...
We also do not know whether "finite colourability problem" is undecidable.
\decisionproblem{
	""Finite Colourability of Automatic Graphs""
}{
	A "presentation" $\•G$ of an "automatic graph" $\?G$.
}{
	Does $\?G$ admit a "colouring" with finitely many colours?
}
\begin{restatable}{conjecture}{conjFiniteColourabilityUndecidable}
	...
	every "finite subgraph" of the "source@@struct" is "$k$-colourable".}
...
and actually this property is decidable by \Cref{prop:order-invariance}
...
\begin{proof}
  ...
  Let $f\colon \?G \to \intInt{1,k}$ be a "colouring" of $\?G$ for some $k\in \N$.
  ...
  Let $k \in \Np$ and let $g\colon \Myc(\?G) \to \intInt{1,k}$ be a
  $k$-colouring of $\Myc(\?G)$.
  Let $g'\colon \Myc(\?G) \to \intInt{1,k}$
...
\begin{proof}[Proof of \Cref{prop:automatic-graph-not-fin-colourable}]
  ...
  \begin{itemize}
    ...
    \item $u$ contains at least one $\bullet$
      ...
      we have $v_i = 1$, and
    \item for all $j \in \intInt{i+1,n}$, $u_j = 0$
    and $v_j \in \2$.
  \end{itemize}
