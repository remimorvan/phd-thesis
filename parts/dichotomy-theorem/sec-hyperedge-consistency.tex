...
\begin{proof}
	...
	And so, there exists $t \in T$ "st" $f(t) = a = b$. We let $\?T_{\?A}$
	be the "subtree" of $\?T$, rooted at $t$, induced by elements
	of $T$ that are sent via $f$ on $\?A$ (resp. $\?B$).
	We define $\?T_{\?A}$ (resp. $\?T_{\?B}$) to be the "substructure" of $\?T$
	induces by elements of $T$ that are sent via $f$ on $\?A$ (resp. $\?B$).
	\begin{claim}
		\AP\label{claim:hyperedge-consistency-uniform-convergence-trees-are-trees}
		$\?T_{\?A}$ and $\?T_{\?B}$ are "trees".
	\end{claim}
	TODO.
	...
	\end{claim}
	The first point is trivial: it suffices to consider the restriction of $f$ to $A$.
