...
which in turns implies the decidability of $\HomRegAutDec{\?B}$.
...
\begin{corollary}[of \Cref{lemma:finite-duality-uniformly-definable-homomorphisms,prop:uniformly-first-order-implies-regular}]
...
that $\HomRegAutDec{\?B}$ is decidable---and even TODO:complexity since $\HomAutDec{\?B}$
is TODO:complexity when $\?B$ has "finite duality".
...
\begin{lemma}[Uniform Convergence of Hyperedge Consistency for Structures with Finite Duality]
	...
	\begin{enumerate}
		...
		\itemAP\label{item:hc-uniform-arbitrary-structures}%
			...
			when $\?A \homto \?B$, and
			$\HCOperator^{\,n}_{\!\?A,\?B}(\topLatticeGuessFunctions{B})(a) = \emptyset$ for some $a\in A$ when $\?A \nothomto \?B$;
		\itemAP\label{item:hc-uniform-first-order-fin-homs}
			$\HomFinClass{\?B}$ has "uniformly first-order definable homomorphisms";
		\itemAP\label{item:hc-uniform-first-order-all-homs}
			$\HomAllClass{\?B}$ has "uniformly first-order definable homomorphisms";
		% \itemAP\label{item:hc-uniform-first-order-class}%
