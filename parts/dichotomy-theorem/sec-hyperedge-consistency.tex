...
We show that under the stronger assumption that $\?B$ has "finite duality",
then this $n$ can be chosen uniformly for all $\?A$'s.
\begin{proposition}[Uniform Convergence of Hyperedge Consistency for Structures with Finite Duality]
	\AP\label{prop:hyperedge-consistency-uniform-convergence}
	Let $\?B$ be a finite "$\sigma$-structure" with "finite duality". Then there exists $n\in\N$ "st" for every arbitrary "$\sigma$-structure", $\HCOperator^n_{\!\?A,\?B}(\topLatticeGuessFunctions{B}) = \HCFixpoint{\?A}{\?B}$.
\end{proposition}

\begin{proof}
	Let $\?D_1, \hdots, \?D_p$ be a finite set of "obstructions" for $\?B$.
	...
	Indeed, let $\?A$ be an arbitrary "$\sigma$-structure" such that $\?A \nothomto \?B$.
	...
	In particular, $\HCOperator^n_{\!\?A,\?B}(\topLatticeGuessFunctions{B})(a)$ for some
	$a \in \?A$.

	We then proceed with the proof of \Cref{prop:hyperedge-consistency-uniform-convergence}.
	Let $m\in \N$ be "st" for all $i \in \lBrack 1,p\rBrack$, $\HCOperator^m_{\!\?D_i,\?B}(\topLatticeGuessFunctions{B}) = \HCFixpoint{\?D_i}{\?B}$.
