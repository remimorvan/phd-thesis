...
\begin{proof}%
    ...
    Consider the "automatic graph" $\AutGraph{V}{\+E}$ for $V \defeq \configs\cdot (\bSymb + \rSymb)$,
	...
	Given a "automatic graph" $\+G = \AutGraph{V}{\+E}$---whose edge relations is given by
    a "automatic automaton" $\relPres{\+E}{\+G}$---and a $k$-coloured automaton $\+B$,
