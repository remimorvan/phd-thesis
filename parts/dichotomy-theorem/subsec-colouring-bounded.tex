...
\begin{proof}
    This is a consequence of the reduction from the "regular $k$-colourability problem" of \Cref{thm:reg-colourability-equiv-separability}, combined with the undecidability of the latter for every $k>1$ (\Cref{thm:k-reg-col-undec}).
\end{proof}
...
\end{lemma}
\begin{proof}
    By reduction from the "regular $2$-colourability problem" on "automatic graphs", which is undecidable by \Cref{thm:k-reg-col-undec}. Let $\tup{V,\+E}$ be an "automatic graph" and $\tup{V,\+E'}$ the symmetric closure of $\tup{V,\+E}$. It follows that $\tup{V,\+E'}$ is still "automatic@@struct" and that there is a "regular $2$-colouring" for $\tup{V,\+E'}$ "iff" there is a "regular $2$-colouring" for $\tup{V,\+E}$---take the same colouring.
    Thus, by \Cref{cor:2reg-2prod}, $\tup{V,\+E}$ is "regularly $2$-colourable" "iff" 
...
\begin{proof}
    ...
    \[
        ...
        \cup \dotsc \cup (\equivclass{A_k}{\autequiv} \times \equivclass{B_k}{\autequiv}).\qedhere
    \]
\end{proof}
