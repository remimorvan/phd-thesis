...
\cite{ElgotMezi1965RelationsGeneralizedAutomata,Nivat1968TransductionChomsky,Berstel1979Transductions,FrougnySakarovitch1993SynchronizedAutomaticRelations,Choffrut2006Survey}. 
...
% \AP ""Automatic relations"" are those definable by multi-head automata, with heads possibly moving asynchronously; "automatic relations" are "automatic relations" that are accepted by multi-head automata whose heads are forced to move synchronously; and 
...
which takes as input an $n$-ary "automatic relation"%
\footnote{
	Recall that "automatic relations" are those definable by multi-head automata,
	with heads possibly moving asynchronously;
	"synchronous relations" are "automatic relations" that are accepted by multi-head automata
...
the "automatic relation" $\+R$ are almost independent of one another.
...
In general, "$\REC$-definability" of "automatic relations" is undecidable, but it becomes decidable 
for two important subclasses: 
deterministic "automatic relations" and "synchronous relations". 
For deterministic "automatic relations",  "$\REC$-definability" has been shown to be decidable in 
...
\AP On the other hand, much less is known about the ""$\RAT$/$\REC$-separability problem"", which takes two $n$-ary "automatic relations" 
...
However, "$\AUT$/$\REC$-separability" is more general than this problem,
...
This conjecture extended a result of Schaefer, who proved that "CSPs" over the Boolean domain\footnote{Meaning that the "target structure" can have only two elements---but recall that $\sigma$ can be arbitrary complex.} was either "P" or "NP"-complete, proven two decades earlier.
...
\AP\begin{proposition}[""Atserias' theorem""]
...
\begin{proposition}[""Larose-Tesson's theorem""]
...
\begin{restatable*}[""Dichotomy Theorem for Automatic Structures""]{theorem}{DichotomyThmDichotomyAutomatic}
...
We conclude in \Cref{sec:dichotomy-discussion}.
