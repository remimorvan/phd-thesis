...
\begin{restatable*}[\introinrestatable{Dichotomy Theorem for Automatic Structures}]{theorem}{DichotomyThmDichotomyAutomatic}
	...
	\begin{description}
		...
		\itemAP[\introinrestatable\itemDTHomDec.] $\HomAutDec{\?B}$ is decidable;
		\itemAP[\introinrestatable\itemDTHomRegDec.] $\HomRegAutDec{\?B}$ is decidable;
		\itemAP[\introinrestatable\itemDTEqual.] $\HomAutDec{\?B} = \HomRegAutDec{\?B}$ "ie" for any "automatic presentation" $\•A$ of a 
		"$\sigma$-structure" $\?A$, there is a "homomorphism" from $\?A$ to $\?B$ "iff" 
		there is a "regular homomorphism" from $\•A$ to $\?B$;
		\itemAP[\introinrestatable\itemDTFirstOrder.] $\HomAllClass{\?B}$ has "uniformly first-order definable homomorphisms".
	\end{description}
