...
In \Cref{sec:uniformly-first-order-definable-hom} we provide
a logic-based proof that if $\?B$ has "finite duality", then $\HomRegAut{\?B} = \HomAut{\?B}$.
In turns, not only does this implies by \Cref{prop:dichotomy-Equal-implies-HomDec}
that $\HomRegAut{\?B}$ is decidable, but actually that the problem is in "NL",
by \Cref{prop:dichotomy-FinDual-implies-HomDec}.
...
refers to the fact that the formulas do not depend on the "source structure".}
...
\begin{example}[{\Cref{ex:zigzag-defn}, continued}]
	...
	On the other hand, looking at the "zigzag graph" of \Cref{ex:zigzag-defn} and \Cref{fig:zigzag-graph}
...
\begin{lemma}
	...
	of \Cref{thm:dichotomy-theorem-automatic-structures} and its converse implication.}
\end{lemma}
...
\begin{proof}
	By \Cref{prop:idempotent-core-preserves-csp-complexity} the problems are "first-order 
	equivalent" and so one is "first-order definable" "iff" the other is.
\end{proof}

\begin{proof}[Proof of \Cref{lemma:finite-duality-uniformly-definable-homomorphisms}]
	...
	"tree duality" (by \Cref{prop:finite-duality-implies-tree-duality}) and so by
	\Cref{prop:charac-Feder-Vardi}, there exists a "homomorphism"
	...
	$\langle \?A_{a,i}, a' \rangle \notFOmodels \unarypred{b_j}(y)$
	...
	\Cref{claim:finite-duality-uniformly-definable-homomorphisms-homomorphism}.
	In turns, using \Cref{claim:finite-duality-uniformly-definable-homomorphisms-homomorphism},
...
\begin{proof}
	...
	and so \[\{u \in \Sigma^* \mid \langle \univStructSynchronous{\Sigma},u \rangle \FOmodels \phi^*_b(x)\}\] is "regular@@lang" by \Cref{prop:automatic-first-order}.
...
(\Cref{prop:dichotomy-general-upper-bounds}), this implies
...
\Cref{fig:informal-HC-algo}.
\begin{marginfigure}
	\centering
	\begin{tikzpicture}
		\node at (0,0) (g) {$g$};
		\node[right= 5em of g] (g2) {$g'$};
		\draw[edge] (g) to node[midway, above] {$\?G$-edge} (g2);
		\node[below= 2em of g] (Img) {$\textrm{Im}_g$};
		\node[below= 2em of g2] (Img2) {$\textrm{Im}_{g'}$};
		\draw[edge] (g) to (Img);
		\draw[edge] (g2) to (Img2);
		\node[below= -.25em of Img] (in) {\rotatebox{90}{$\in$}};
		\node[below= -.25em of Img2] (in2) {\rotatebox{90}{$\in$}};
		\node[below= -.25em of in] (h) {$\forall h$};
		\node[below= -.25em of in2] (h2) {$\exists h'$};
		\draw[edge] (h) to node[midway, below] {$\?H$-edge} (h2);
	\end{tikzpicture}
	\caption{
...
\begin{example}[{\Cref{ex:zigzag-FO}, continued}]
	...
	if $\transitiveTournament{2}$ and the "source structure" is $\zigzag{n}{2}$.
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\input{tikz/dichotomy/zigzag-graph-HC-T2-step0}
	\end{tikzpicture}\\[2em]
	\begin{tikzpicture}
		\input{tikz/dichotomy/zigzag-graph}
		% \input{tikz/dichotomy/2-transitive-tournament}
		\input{tikz/dichotomy/zigzag-graph-HC-T2-step1}
	\end{tikzpicture}\\[2em]
	\begin{tikzpicture}
		\input{tikz/dichotomy/zigzag-graph}
		% \input{tikz/dichotomy/2-transitive-tournament}
		\input{tikz/dichotomy/zigzag-graph-HC-T2-step2}
	\end{tikzpicture}
...
\sidenote[][-3em]{Note that in this part, while some results---mostly complexity/decidability ones---require the assumption that the "source structure" is finite, some results do not,
...
\begin{proposition}[{\cite[Theorem~3.3]{LaroseLotenTardif2007CharacterisationFOCSP}}]
...
has "tree duality" by \Cref{prop:finite-duality-implies-tree-duality}.

\begin{figure}
	\centering
	\begin{tikzpicture}
		\input{tikz/dichotomy/zigzag-graph}
		\input{tikz/dichotomy/zigzag-graph-HC-P2-step2}
	\end{tikzpicture}
	\quad\emph{(step $2$)~}\\[2em]
	\begin{tikzpicture}
		\input{tikz/dichotomy/zigzag-graph}
		\input{tikz/dichotomy/zigzag-graph-HC-P2-step3}
	\end{tikzpicture}
	\quad\emph{(step $3$)~}\\[2em]
	\begin{tikzpicture}
		\input{tikz/dichotomy/zigzag-graph}
		\input{tikz/dichotomy/zigzag-graph-HC-P2-step6}
	\end{tikzpicture}
	\quad\emph{(step $6$)~}\\[2em]
	\begin{tikzpicture}
		\input{tikz/dichotomy/zigzag-graph}
		\input{tikz/dichotomy/zigzag-graph-HC-P2-step7}
	\end{tikzpicture}
	\quad\emph{(step $7$)~}\\[2em]
	\begin{tikzpicture}
		\input{tikz/dichotomy/zigzag-graph}
		\input{tikz/dichotomy/zigzag-graph-HC-P2-step13}
	\end{tikzpicture}
	\quad\emph{(step $13$)}
	\caption{%
...
\begin{example}[{\Cref{ex:zigzag-HC-T2}, continued}]
	...
	While $\pathGraph{2}$ does not have "finite duality" (\Cref{ex:zigzag-defn}), it has
	"tree duality" (\Cref{prop:2-path-tree-duality}), and so the "hyperedge consistency algorithm@@finite"
	...
	on the "source structure" $\zigzag{5}{2}$.
	...
	In general, on "source structure" $\zigzag{n}{2}$ (with $n\in\Np$),
...
\begin{corollary}
	...
	polynomial time using the algorithm of \Cref{fig:hc-finite}.
\end{corollary}

However, note that the example of $\clique{2}$ shows that the property
of having "tree duality", is not necessary for $\HomFin{\?B}$ to be decidable in "P".
% Finally, we will prove a proposition that we will prove useful later in \todo{TODO}.
% \begin{proposition}
...
% 	this is false if $\?B$ does not have "tree duality". For instance,
...
\begin{figure}
	...
	\begin{algorithm}[H]
		...
		\KwInput{Two "finite $\sigma$-structures" $\?A$ and $\?B$.}
		$\textrm{Im}^{0}_a \leftarrow B$ for $a\in A$\;
		$n \leftarrow 0$\;
		\Do{%
			some $\textrm{Im}_a$ has been updated
		}{
			\For{$a \in A$}{
				\nosemic$\textrm{Im}^{n+1}_a \leftarrow \{ b \in \textrm{Im}^{n}_a \mid$\;
				\pushline$\forall \langle a_1,\, \dotsc,\, a_{k-1} \rangle \in \adjacency{a}{\?A}{\+R}{i},$\;
				$\exists \langle b_1,\, \dotsc,\, b_{k-1} \rangle \in \adjacency{b}{\?B}{\+R}{i},$\;
				$b_1 \in F(a_1) \land \dotsc \land b_{k-1} \in F(a_{k-1})$\;
				\popline\dosemic$\}$\;
				\If{$\textrm{Im}^{n+1}_a = \emptyset$}{
					\Return{false}\;
				}
			}
			$n \leftarrow n+1$\;
...
\begin{proof}
	...
	We assume that $\?A\homto \?B$: by \Cref{claim:hyperedge-consistency-ball,prop:hyperedge-consistency-antimonotonicity}
	...
	which yields that $\HCOperator^{\,n}_{\!\?A,\?B}(\topLatticeGuessFunctions{B}) = \HCFixpoint{\?A}{\?B}$. Together with the contraposition of \Cref{coro:HC-empty-implies-no-hom},
	...
	"st" $\?T \homto \?A$.
	By \Cref{prop:finite-duality-implies-tree-duality}, "wlog" $\?T$ is a "$\sigma$-tree".
	...
	\begin{marginfigure}
		\centering
		\begin{tikzpicture}
			\input{tikz/dichotomy/proof-of-hyperedge-consistency-uniform-convergence-hom}
		\end{tikzpicture}
		\caption{
			\AP\label{fig:proof-of-hyperedge-consistency-uniform-convergence-hom}
			Construction of $\?C$ as in the proof of \Cref{claim:hyperedge-consistency-uniform-convergence-hom} when $\?A$
	...
	vertex $b$ will be removed as a potential image for $a$ since $b \not\in \HCFixpoint{\?A}{\?B}(a)$, but because of the copy of $\?B$ included in $\?C$, any "homomorphism" from $\?C$ to $\?B$ must map $a$ to $b$. See \Cref{fig:proof-of-hyperedge-consistency-uniform-convergence-hom}
	for an example.
	% \begin{claim}
	...
	Since $\?B$ has "finite duality", it is "rigid" by \Cref{{prop:finite-duality-implies-rigid}}, and so in particular $f(b) = b$.
	...
	namely the smallest congruence containing $\{\tup{t_1,t_2} \mid f(t_1) = f(t_2)\}$.
	Then, we let $\?U_{\?A}$ be the "substructure" of $\?U$
...
Putting \Cref{lem:hyperedge-consistency-preserves-regularity}
(for computations) and \Cref{lem:hyperedge-consistency-uniform-convergence}
(for correctness), we get (1) that the
algorithm of \Cref{fig:hc-automatic} decides $\HomRegAut{\?B}$
...
\begin{figure}
	...
	\begin{algorithm}[H]
		\SetAlgoLined
		\SetKwRepeat{Do}{do}{while}
		\SetKwInput{KwInvariant}{Invariant}
		\KwInput{An "automatic presentation" $\+A$ of a "$\sigma$-structure" $\?A$ and
		a "finite $\sigma$-structure" $\?B$.}
		\KwInvariant{First-order formulas $\phi_{Y}(x)$ ($Y \subseteq B$)
		"defining@first-order definable" the set of $a \in A$ "st"
		$\textrm{Im}_a = Y$, where $\textrm{Im}_a$ is as in \Cref{fig:hc-finite}.}
		$\phi^{0}_B(x) \leftarrow \top$\;
		$\phi^{0}_Y(x) \leftarrow \bot$ for all $Y \subsetneq B$\;
		$n \leftarrow 0$\;
		\Do{%
			some $\phi_{Y}(x)$ has been semantically updated
		}{
			compute $\tup{\phi^{n+1}_{Y}(x)}_{Y \subseteq B}$ from the 
			$\tup{\phi^{n}_{Y}(x)}_{Y \subseteq B}$ using
			\Cref{lem:hyperedge-consistency-preserves-regularity}\;
			\If{$\semFO{\phi^{n+1}_\emptyset(x)}{\?A} \neq \emptyset$}{
				\Return{false}\;
			}
			$n \leftarrow n+1$\;
		}
		\Return{true}
	\end{algorithm}
	\caption{\AP\label{fig:hc-automatic} The ""hyperedge consistency algorithm for automatic structures"".}
\end{figure}
...
% Indeed, let $\?D_1, \dotsc, \?D_p$ be a finite set of "obstructions" for $\?B$.
% For each $i\in\intInt{1,p}$, since $\?D_i \nothomto \?B$, there exists
% $n_i\in\N$ "st" $\HCOperator^{n_i}(\topLatticeGuessFunctions{B})(d_i) = \emptyset$ for some $d_i\in D_i$. We let $n \defeq \max_{i\in\intInt{1,p}} n_i$.

% Then let $\?A$ be an arbitrary "$\sigma$-structure" such that $\?A \nothomto \?B$.
% Then $\?D_i \homto \?A$ for some $i \in \intInt{1,p}$. 
