...
\todo{We discuss this in more details in \todo{TODO}.}}
...
\begin{proof}[Proof of \Cref{lemma:finite-duality-uniformly-definable-homomorphisms}]
	...
	and let $\langle a_1,\,\dotsc,\,a_l \rangle \in \+R(\?A)$. Then for each $i \in \lBrack 1,l\rBrack$, for every $b_i \in F(a_i)$, there exists a "homomorphism" $f$ from $\?A$ to $\?B$
...
\begin{proof}
	...
	then there exists $b_1 \in F(a_1)$, $\dotsc$, $b_{k-1} \in F(a_{k-1})$ "st" 
	$\langle b_1,\, \dotsc,\, b_{k-1} \rangle \in \adjacency{b}{\?B}{\+R}{i}$.
	Now, let $\+R_{(k)} \in \sigma$ and $i \in \lBrack 1,k\rBrack$,
...
\begin{proof}
	...
	for any $i\in\lBrack 1,k\rBrack$, for any $Y_1,\dotsc,Y_{k-1} \subseteq B$, whether there
	...
	Since $\?B$ has "finite duality", it is "rigid" by \todo{ADDREF}, and
