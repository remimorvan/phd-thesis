...
\todo{We discuss this in more details in TODO.}}
...
\begin{proof}[Proof of \Cref{lemma:finite-duality-uniformly-definable-homomorphisms}]
	...
	then $\?B$ has "finite duality", and in particular it has \todo{addref}
	"tree duality" and so by \todo{addref}, there is a "homomorphism"
...
Yet, \todo{who} managed to identify a necessary and sufficient condition on $\?B$ for
...
has "tree duality" by \Cref{prop:finite-duality-implies-tree-duality} and \todo{ADDREF}.
...
\begin{example}[{\Cref{ex:zigzag-HC-T2}, continued}]
	...
	"tree duality", by \todo{ADDREF}, and so the "hyperedge consistency algorithm@@finite"
...
\begin{corollary}
	...
	polynomial time.\todo{ Precise complexity.}
\end{corollary}
...
\todo{somewhere \AP""hyperedge consistency@@finite"" algo.}
...
\begin{proof}
	...
	\todo{CONCLUDE SOMEWHERE THAT CONSTRUCTION IS NOT ELEMENTARY.}}
	\begin{align*}
		\chi_b(x) \defeq\; &
...
\begin{proof}
	...
	More precisely, it only depends on, for any "predicate" $\+R$ of arity $k$,
	for any $i\in\intInt{1,k}$, for any $Y_1,\dotsc,Y_{k-1} \subseteq B$, whether there
	are elements $\langle a_1,\,\dotsc,\,a_{k-1}\rangle \in \adjacency{a}{\?A}{\+R}{i}$
	"st"  
	$\langle F(a_1),\,\dotsc,\,F(a_{k-1}) \rangle = \langle Y_1,\,\dotsc,\,Y_{k-1} \rangle$.
	\todo{Diego says ``I'm not sure what you want to say here. Why the Yi's? There's something missing?'' about the previous sentence. Clarify this.}
	...
	\todo{add figure ($\?B$ is $\transitiveTournament{2}$, $\?A$ is 2-path.)}
	% \begin{claim}
	...
	of $\?A$ that is adjacent to $\?B$ in $\?C$, and since $\?T$ is "connected" by
	\todo{ADDREF},
...
\todo{ \AP""hyperedge consistency algorithm for automatic structures""}
