...
\begin{definition}[Types and Profiles]
    ...
    \begin{itemize}
        ...
        \itemAP otherwise, when $b$ is "non-atomic", we assign to each variable
            $z \in \bagmap(b) \subseteq \vertex{\alpha}$ a ""type""\phantomintro{\typeStw}
            \begin{align*}
...
\begin{figure}[tbp]
    ...
    \subfloat[The ``new'' "refinement" $\rho'$ of $\gamma$.]{%
        ...
        \includegraphics[width=.35\linewidth]{trio-refinement-result.pdf}
    }
	\hfill
    \subfloat[The ``new'' "approximation" $\alpha'$ of "tree-width" 2.]{
        \AP\label{fig:trio-result-approx}
        \includegraphics[width=.35\linewidth]{trio-approximation-result.pdf}
    }
	\caption{
