\section[Proof of the Key Lemma]{Key Lemma: Maximal Under Approximations are Semantically Finite}
\label{sec:proof-key-lemma}

\AP We can now start to describe the constructions used to prove the "Key Lemma". Given a fixed "C2RPQ" $\gamma$ and a fixed $k \geq 1$,
...
%         \textcolor{red}{Attention: use "contraction@@2w"}
%     }
% \end{figure}
\begin{figure*}[tbp]
	\centering
...
\begin{definition}[Types and Profiles]
    ...
    \begin{itemize}
        ...
        \itemAP otherwise, when $b$ is "non-atomic", we assign to each variable
            $z \in \bagmap(b) \subseteq \alpha$ a ""type@@stw""\phantomintro{\typeStw}
            \begin{align*}
...
part simply relies on an enhanced ``pigeon-hole principle'' (\Cref{fact:pigeon-hole}).
...
\subsection{Proof of Lemma~\ref{lemma:bound_size_refinements}}
