\section{Semantic Tree-Width for Simple Queries}
\label{sec:sre}
...
a pair $\tup{\?T, \bagmap}$ with $\bagmap: T \to \pset{\vars{\gamma}}$
such that:
\begin{itemize}
...
\subsection{Semantic Tree-Width Problem}
...
%   \end{restatable}
  ...
  \begin{proof}
    ...
    Take any such counterexample $\anexpansion$ of minimal size (in number of atoms). We show that for any "(two-way) internal path" of $\anexpansion$ of the shape
