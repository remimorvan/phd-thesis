...
We define \AP""two-way contractions"" (or \reintro(2w){contractions} for short in this chapter) analogously to "one-way contractions" by redefining
the notion of "internal variable".
In the context of one-wayness, it designated any variable with both in-degree and out-degree one.
Now, for a variable to be "internal@internal variable", we only ask that its total degree is two.
Concretely, this means that "two-way contractions" can also contract paths
like $x \atom{K} y \coatom{L} z $
into a single "atom" $x \atom{KL^{-1}} z$.%
\footnote{
	Naturally, we can equally "contract@@2w" 
	$x \coatom{K} y \atom{L} z$ into the "atom" $x \atom{K^{-1}L} z$.
}
...
Our approach to proving both \Cref{thm:decidability-semtw,thm:closure-under-sublanguages}
