\section{Acyclic Queries: the Case $k=1$}
...
\subsection{Contracted Tree-Width}
...
Recall that a "one-way contraction" of a "CRPQ" is defined analogously to a "(two-way) contraction@contraction@2w", except that we only contract "one-way internal paths".

\begin{definition}
	\AP
	Define the ""contracted tree-width"" (resp.\ ""one-way contracted tree-width"")
	of a "C2RPQ" as the minimum of the "tree-width" among its "contractions@@2w" (resp.\ of its "one-way contractions"). Let $\intro*\ContrTw$ and $\intro*\ContrTwOneWay$ be, respectively, the set of all "C2RPQs" of "contracted tree-width" at most $k$ and of "CRPQs" of "one-way contracted tree-width" at most $k$.
\end{definition}
...
can be "contracted@@2w" into $x_0 \atom{LM^{-}} x_1$. On the other hand,
...
\begin{proof}
	\begin{align*}
		\MUA{\gamma}{\Tw}
			...
			\quad\text{since "contractions@@2w" preserve semantics,} \\
...
\subsection{The Key Lemma for Contracted Tree-Width One}
...
\begin{proof}
	...
	"\emph{contracted} tree-width@contracted tree-width" 1. We now apply the constructions not to a decomposition of $\alpha$ but to a "fine tagged tree decomposition" of a \emph{"contraction@@2w"} of $\alpha$ of "tree-width" 1.
	
	\begin{fact}
		Let $\gamma$ be a "C2RPQ", $\chi$ be a "contraction@@2w" of $\gamma$, and
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item If $\Gamma \semequiv \MUAHomBounded{\Gamma}{\ContrTwOneWay[1]}{\leq\lOne}$, 
			...
			most 1, and hence (by "contraction@@2w") it is equivalent a "UCRPQ" of "tree-width" at most 1. 
	\end{itemize}
