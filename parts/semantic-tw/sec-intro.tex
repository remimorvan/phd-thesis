...
In this chapter, we consider the problem of, given a "C2RPQ", or a "union of C2RPQs", deciding
if it is equivalent to a "query@C2RPQ" of small "tree-width".
...
Similar statements of the following proposition can be considered
folklore (see "eg" {\cite[Theorem IV.3]{RomeroBarceloVardi2017Homomorphism}}).
It can be proven in the same fashion as \Cref{prop:eval-CQ-bounded-tw}.
\begin{proposition}
	...
	or $\+O(\size{\Gamma} \cdot |G|^{k+1})$ using a RAM model, where $\Gamma$ and $G$ are the input "UC2RPQ" and "graph database", respectively.
\end{proposition}
...
\begin{example}
	...
	\!\footnote{This example uses exactly the same idea as
	\Cref{prop:unionsmatter}.}
...
\label{sec:relwork}
\todo{WTF? WHY "coNP" and not "NP"?}
On the class of "conjunctive queries", the "semantic tree-width $k$ problem" becomes the "coNP"-complete problem of finding out whether a query has a retraction of "tree-width"
at most $k$. In fact, "CQs" enjoy the effective existence of unique minimal queries \cite[Theorem 12]{ChandraMerlin1977Implementation} (the "core"), which happen to also minimize the tree-width. For "CRPQs" and "UC2RPQs", the question is far more challenging, and it has only been solved for the case $k = 1$ 
...
Furthermore, having bounded "semantic tree-width" precisely describes
the classes of "CQs" that have a tractable (both in the sense of polynomial time
and "FPT") "evaluation problem" \cite[Theorem~1.1]{Grohe2007ComplexityHomomorphism}.
...
A "maximal under-approximation" of tree-width at most $k$ of a "CQ" $\gamma$ consists of a
...
the case for the class of "simple regular expressions", which will be our focus of study in \Cref{sec:sre}. Moreover, even if some class $\+L$
...
\begin{restatable*}[Existence of the "maximal under-approximation"]{corollary}{muaexistseffective}
...
\begin{restatable*}[FPT evaluation for bounded "semantic tree-width"]{corollary}{fptEvalBoundedSemTreeWidth}
	\AP\label{coro:fpt-eval-bounded-semtreewidth}
	For each $k\geq 1$, the "evaluation problem" for "C2RPQs" of "semantic tree-width"
	at most $k$ is fixed-parameter tractable---"FPT"---when parameterized in the size of
...
\begin{restatable*}{theorem}{paraNLEvalBoundedSemPathWidth}
	...
	"semantic path-width" at most $k$ is in "para-NL" when parameterized in the size of the query.
...
Finally, we conclude in \Cref{sec:discussion-stw}.
...
questions (\Cref{sec:discussion-stw-complexity}) to extensions of our results to bigger classes
or larger settings (\Cref{sec:discussion-stw-larger-classes,sec:discussion-stw-different-notions}).
