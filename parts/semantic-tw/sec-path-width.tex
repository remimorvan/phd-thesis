\section{Semantic Path-Width}
\label{sec:semantic-path-width}
% \diego{``Deciding Boundedness'' n'est pas ce qu'on fait...}

In this section, we extend our results to "path-width". Our motivation lies in the fact that "UC2RPQs" of bounded "semantic path-width" admit a "paraNL"\footnote{This is the parameterized counterpart of non-deterministic logspace.} algorithm for the 
"evaluation problem"---see \Cref{thm:evaluation-bounded-pathwidth}---to be compared with "FPT" for bounded "semantic tree-width".

\subsection{Path-Width of Queries}
...
"para-NL" when parameterized in the size of the query (\Cref{thm:evaluation-bounded-pathwidth}).

\subsection{Deciding Bounded Semantic Path-Width}
...
\begin{definition}
	...
	of a "C2RPQ" as the minimum of the "path-width" among its "contractions@@2w" (resp.\ of its "one-way contractions"). Let $\intro*\ContrPw$ and $\intro*\ContrPwOneWay$ be, respectively, the set of all "C2RPQs" of "contracted path-width" at most $k$ and of "CRPQs" of "one-way contracted path-width" at most $k$.
\end{definition}
...
\begin{proof}
	\begin{align*}
		\MUA{\gamma}{\PwOneWay}
			...
			\quad\text{since "contractions@@2w" preserves the semantics,} \\
...
\begin{proof}
	...
	to a suitable "path decomposition" of a "contraction@@2w" of $\alpha$ of width $k$.
\end{proof}
...
\subsection{Evaluation of Queries of Bounded Semantic Path-Width}
...
We define \AP""para-NL"" as the class of parameterized languages $L \subseteq \Sigma^* \times \N$
...
finite structures, when parameterized by the maximum degree of the structure
