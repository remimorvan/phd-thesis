...
\section{Alternative Upper Bound for Containment of UC2RPQs}
...
\begin{proposition}
	...
	The "containment problem" $\Gamma \contained \Delta$ for "UC2RPQs" can be solved in non-deterministic space $2^{c\cdot \size{\Gamma}} + p_{\Delta} \cdot 2^{c\cdot m_\Delta}$, where $m_\Delta$ is the size of the greatest disjunct of $\Delta$, namely $m_\Delta = \max{\{\size{\delta_\Delta} \mid \delta \in \Delta\}}$, $p_\Delta$
...
\section{Path-Width is not Closed under Refinements}
