...
For a more general introduction to different query languages for "graph databases",
including "CRPQs", see \cite{Barcelo2013Querying}, and for a more practical approach,
...
\begin{proof}
    Assume by contradiction that this would be the case. Clearly, $\gamma_2(x,y)$
...
transitive closures of tables---or of projections of tables---
...
where $\intro*\edges{G} \subseteq G \times \A \times G$
...
$x \atom{ (ab)^*} y$ asks that there is a path from $x$
...
\begin{definition}[CRPQ: syntax]
    ...
    \footnote{In practice, $L$ is specified as a regular expression. For all results
    of this thesis, however, we assume that it is represented as an NFA. This makes
    little difference as both representations are equivalent up to a polynomial
    blow-up.}
    We denote by \AP$x \intro*\atom{L} y$ the triple $\tup{x,L,y}$,
    called \reintro{atom}. The set $\intro*\atoms{\gamma}$ of "atoms" is denoted conjunctively.
    We denote by $\intro*\vars{\gamma}$ the set of variables occurring in $\gamma$. As for
    "CQs", variables of $\bar x$ are called \AP""output variables"".
\end{definition}
...
\]
which intuitively asks for, in a given "graph database",
...
$f\colon \gamma(\bar x) \to \tup{\?H, \bar v}$ is an "evaluation map",
...
an "evaluation map".%
\begin{fact}
    \AP\label{fact:prelim-db-crpq-closed-under-hom}
    In particular, the semantics of "CRPQs" is
    "closed under homomorphisms".
\end{fact}
When $\gamma(\bar x)$ is a "CQ", an "evaluation map" $\gamma(\bar x) \to \tup{\?H, \bar v}$
exactly corresponds to "homomorphisms" $\tup{\?G, \bar x} \to \tup{\?H, \bar v}$,
...
to the theory of "CQs": one reason is first that the construction
...
From a very simple "structure" of "path-width" 1, we can obtain a "structure" of
unbounded "tree-width"!
Hence, "CRPQs" deserve to be studied for their own sake.
...
the sum of the size of automata (NFAs) used to describe $\gamma$.
...
\begin{proof}
    ...
    The upper bound can be proven by (1) guessing a function from the variables
    of the "query@CRPQ" to the vertices of the "database@@graph",
    and (2) checking that it is an "evaluation map". 
    For each "atom", each check consists in verifying if there is a
    path from some vertex to another one that is labelled by a word of some language,
...
In the other direction, the languages used in "CRPQs" can be extended to navigate edges in both directions. 
...
\begin{proof}
    ...
    \[
        \bigvee \{
            \gamma \mid
            \gamma \text{ "canonical CQ" of some } \?G \in \phi
        \}.\qedhere
    \]
\end{proof}
...
\footnote{\emph{Déjà vu?} See the paragraph before \Cref{fact:prelim-db-crpq-closed-under-hom}.}
...
While this is indeed an issue, it only illustrates part of the difficulty
...
we look at every possible choice of words $\tup{u_i \in L_i}_{i\in \intInt{1,n}}$.
...
\begin{enumerate}[label=\roman*.]
	...
	\item $\{a^{-}\}$ if $a^{-} \in \A^{-}$ belongs to $\subaut{\+A_L}{q_i}{q_{i+1}}$.
\end{enumerate}
...
\begin{definition}
    ...
    \]
    such that $\textcolor{maincolor}{K = \+A_L[q_i,q_j]}$.
	\begin{fact}
...
Note that any "atom $m$-refinement" is, by definition, also an
"atom $m'$-refinement" when $m \leq m'$: as a consequence, in the "refinement" of a "C2RPQ"
...
\begin{proposition}[Folklore]
    \!\footnote[][-3cm]{The proof is elementary,
    ...
    \footnote[][-3cm]{The proposition works for "Boolean" as well as "non-Boolean" queries:
...
\begin{proof}[Proof sketch]
    ...
    We then look at how $\gamma_2$ maps into $\?G_1$ in the following sense:
    ...
    By the pigeon-hole principle, if $n$ has at least double-exponential size, then there must exist
...
\begin{proposition}
    \!\footnote{Proven independently in \cite[Theorem 5]{CalvaneseDeGiacomoLenzeriniVardi2000Containment} for "C2RPQs"
    ...
    The \AP""containment problem"" for "UC2RPQs" is "ExpSpace"-complete. The lower bound already
...
and relies on encoding counters with exponential value in polynomial space.

\subsection{Queries Over Simple Languages}
\label{sec:prelim-db-sre}

The high complexity of \Cref{prop:containment-figueira,prop:containment-expspace-complete} motivates
...
\AP A ""simple regular expression"", or \reintro{SRE}, is a regular expression of the form $a^*$ for some letter $a \in \A$ or of the form $a_1 + \dotsb + a_m$ for some $a_1, \dotsc, a_m \in \A$. 
...
Moreover, \UCRPQSRE{} also corresponds to "UC2RPQs" whose languages are expressed via "SREs";
