...
\begin{itemize}
	...
	\item $\neg$, $\lor$, $\land$, $\exists$ and $\forall$ are interpreted as the Boolean operators of negation, disjunction and conjunction, and as the existential and universal quantifiers, respectively.
\end{itemize}

Given a "$\sigma$-structure" $\?A$, we denote by \AP$\intro*\semFO{\phi(\bar x)}{\?A}$
...
We let $\Sigma_0$ and $\Pi_0$ be the sets of quantifier-free formulas.
...
those that do not contain the negation nor universal quantification are called \AP""existential positive formulas"" and lastly, those that do not contain negation nor any type of quantification
...
with their semantics. Lastly, \AP""monadic second-order logic"" is obtained from "first-order logic"
...
\begin{itemize}
	\itemAP \todo{""Connectivity in Finite Graphs""}
	\itemAP \todo{""Reachability in Finite Graphs""}
	\itemAP \todo{\cite{Schmitz2016ComplexityHierarchies} for "Tower" $\intro*\tower$}
\end{itemize}