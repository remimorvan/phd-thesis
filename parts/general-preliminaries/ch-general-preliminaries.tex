...
for instance, given a set $X$, we use Roman lowercase letters ($x,y,z,\dotsc$)
to denote elements of $X$, Roman uppercase letters for its subsets ($A, B, C, \dotsc$),
...
$\alpha$ for one of its "\textbf{a}pproximations@approximations",
...
\begin{itemize}
	...
	\itemAP ""symmetric@@rel"" when $\tup{x,y} \in \+R$ "iff" $\tup{y,x} \in \+R$
		for all $x,y$;
	\itemAP ""transitive@@rel"" when $\tup{x,y} \in \+R$ and $\tup{y,z} \in \+R$
		imply $\tup{x,z} \in \+R$
		for all $x,y,z$.
\end{itemize}
...
Over other "signatures", we apply the same construction but then identify the constants
in both structures.%
...
\begin{marginfigure}
	\centering
	\begin{tikzpicture}
		\input{tikz/general-prelim/product-block.tex}
	\end{tikzpicture}
	\caption{
...
The \AP""diameter"" of a "structure" is the maximum over $u$ and $v$
of the "distance@@struct" between vertices $u$ and $v$.

The ball \AP$\intro*\ball{\?A}{a}{m}$ centred at vertex $a\in A$ and of radius $r\in\N$ of
...
\begin{marginfigure}
	\centering
	\begin{tikzpicture}
		\input{tikz/general-prelim/directed-tree.tex}
	\end{tikzpicture}
...
and the natural "interpretation@@predicates" of the "predicates" and constants.
...
is sent on itself.
\begin{proposition}
	Any "finite $\sigma$-structure" $\?A$ admits a unique%
	\footnote{Throughout this thesis, by ``unique'' we mean ``unique up to isomorphism''.}
	minimal---in the number of vertices---retraction.
\end{proposition}
...
\begin{proof}
	...
	By composition, we obtain a surjective "homomorphism" from $\?B_1$ to itself and from $\?B_2$ to itself. By finiteness, these surjective "homomorphisms" must actually be "automorphisms".
...
\begin{proposition}
	...
	and only if, their "cores" are "isomorphic".
\end{proposition}
...
A  \AP""Boolean"" ""(semantical) query"" is any subclass of the
...
\]
where the $x_i$'s range over the set of variables and $\+R_{(k)}$ over the "signature" $\sigma$.
...
and a "pointed $\sigma$-structure" $\tup{\?A,\bar a}$ where the arity of $\bar a$ coincides with
...
and we define a few important fragments of this logic in \Cref{tab:fragments-exist-FO}
...
We assume the reader to be familiar with the basic notions of automata theory.
...
The signature of words over an alphabet $\Sigma$ has a binary "predicate" $\preceq$
...
any monoid morphism $\phi\colon \Sigma^* \to M$ yields both a monoid
left action and a monoid right action of $\Sigma^*$ over a set $M$,
with the properties that $u \cdot (x \cdot v) = (u \cdot x) \cdot v$.
...
consists of \emph{stream}, "ie" a function $\+V$ from alphabets to languages over this alphabet---for which the image of an alphabet $\Sigma$ is denoted by
$\+V_{\Sigma}$ rather than $\+V(\Sigma)$---, and with the following properties:%
...
This result generalizes Schützenberger's famous theorem, showing that
...
\sidenote[][-2cm]{\blockquote[Gilles Dowek, Laurence Devillers \& Serge Abiteboul, \emph{Qui a hacké Garoutzia}]{Mais dans ce sandwich absurde entre mon initialisation et ma terminaison, que m'aura-t-il vraiment manqué ? Peut-être un corps… un corps qui ressent et tutti quanti !}}
...
We now turn to undecidable classes: all hardness results are considered up to
many-one reductions, "ie" computable functions preserving both membership and non-membership
to the problem.
...
be a "directed path@$k$-path" \cite[Theorem 4.2]{Etessami1997CountingLogSpace}!

\subsection{Parametrized Complexity}

A parameterized language is any subset $L \subseteq \Sigma^* \times \N$:
the first component represents an instance, and the second a parameter.
We define \AP""FPT"" as the class of \reintro{fixed-parameter tractable}
problems, for which there is a "Turing machine" $\+M$ "st"
...
and, moreover, $\+M$ runs in time
$\+O(f(\textcolor{cBlue}{k})\cdot |\textcolor{cRed}{x}|^c)$,
for constant $c$ that does not depend on $\textcolor{cRed}{x}$ nor $\textcolor{cBlue}{k}$,
and where $f\colon \N \to \N$ is a computable function.

""XP"" is the class of slicewise polynomial-time problems $L \subseteq \Sigma^* \times \N$:
"st", for every bounded value of the parameter,
then the resulting language must be in "P".
Formally, for each $k\in\N$, we ask that the (classical) problem 
\[
\{ u \in \Sigma^* \mid \tup{u,k} \in L \}
\]
belongs to $P$.

Note that "FPT" is, by construction, included in "XP".
However, an algorithm that runs in time $\+O(|\textcolor{cRed}{x}|^{\textcolor{cBlue}{k}})$
is a typical example of algorithm that is "XP" but not "FPT".

The class \AP""W1"", which we will not formally define here,
can roughly be seen as the equivalent of "NP". See \cite{CyganEtal2015ParametrizedBook}
for a formal definition.
Some problems in "XP" belong to "FPT", and are considered to be tractable,
"ie" efficiently solvable in practice. On the other hand, others problems,
such as the $k$-clique problem, when parameterized by $k$,
are in "XP" but are "W1"-hard, and hence conjectured not to be in "FPT".
We think of these problems as non-tractable.