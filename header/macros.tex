...
\newrobustcmd{\proofsubcase}[1]{\adforn{39}\adforn{39}~\emph{#1}~}
...
\newenvironment{abstract}[1][Abstract]{
	\begin{minipage}{.75\linewidth} \begin{center}\large\textsc{#1}\end{center}
}{
	\end{minipage}
}
...
\newrobustcmd\smashxleftrightarrow[1]{%
	\raisebox{-.04em}{$%
		\xrightleftharpoons{\smash{\raisebox{-.1em}{%
			\tiny{#1}%
		}}}%
	$}%
}%
...
\newrobustcmd\introinrestatableopt[1]{%
	\ifmainstatement%
		\intro[#1]{#1}%
	\else%
		\kl[#1]{#1}%
...
\medskip
\begin{tcolorbox}[colback=cYellow!20!white, colframe=cYellow]
	\textsc{Hypothesis.}
}{%
	\end{tcolorbox}
	\medskip
...
\newenvironment{hypothesisnotation}{%
\medskip
\begin{tcolorbox}[colback=cYellow!20!white, colframe=cYellow]
	\textsc{Hypothesis/Notation.}
}{%
	\end{tcolorbox}
	\medskip
}